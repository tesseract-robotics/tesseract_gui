...
#ifndef TESSERACT_IGNITION_RENDERING_MINIMAL_SCENE_HH
#define TESSERACT_IGNITION_RENDERING_MINIMAL_SCENE_HH
...
namespace tesseract_gui
{
  ...
  class MinimalScene : public QObject
  {
    Q_OBJECT
...
//        override;
  ...
  class IgnRenderer
  {
    ...
    public: void NewHoverEvent(const ignition::math::Vector2i &_hoverPos);

    /// \brief New hover event triggered.
    /// \param[in] _dropText Text dropped on the scene
    /// \param[in] _dropPos Mouse drop screen position
    public: void NewDropEvent(const std::string &_dropText, const ignition::math::Vector2i &_dropPos);

    /// \brief Handle key press event for snapping
    /// \param[in] _e The key event to process.
    public: void HandleKeyPress(const ignition::common::KeyEvent &_e);

    /// \brief Handle key release event for snapping
    /// \param[in] _e The key event to process.
    public: void HandleKeyRelease(const ignition::common::KeyEvent &_e);
    ...
    /// \brief Ambient color
    public: ignition::math::Color ambientLight = ignition::math::Color(0.3f, 0.3f, 0.3f, 1.0f);
...
//        const ignition::math::Vector2i &_dropPos);
    ...
    /// \return Updated node.
    private: QSGNode *updatePaintNode(QSGNode *_oldNode, QQuickItem::UpdatePaintNodeData *_data) override;
...
#endif // TESSERACT_IGNITION_RENDERING_MINIMAL_SCENE_HH
