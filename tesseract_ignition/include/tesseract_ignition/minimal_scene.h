...
private:
  ::ignition::utils::UniqueImplPtr<Implementation> dataPtr;
};
...
public:
  explicit RenderWindowItem(QQuickItem* _parent = nullptr);
...
public:
  ...
  /// the render window.
public:
  Q_INVOKABLE void OnHovered(int _mouseX, int _mouseY);
...
public:
  Q_INVOKABLE void OnDropped(const QString& _drop, int _mouseX, int _mouseY);
...
public:
  explicit TextureNode(QQuickWindow* _window, RenderSync& _renderSync);
