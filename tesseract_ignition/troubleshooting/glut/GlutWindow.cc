...
void displayCB()
{
  ...
  tmp.SavePNG("/tmp/glut_example_image.png");
  ...
  {
    GLfloat glmvm[16];
    glGetFloatv(GL_MODELVIEW_MATRIX, &glmvm[0]);
    std::stringstream ss;
    ss << "GL_MODELVIEW_MATRIX" << std::endl;
    for (int r = 0; r < 4; ++r)
    {
      for (int c = 0; c < 4; ++c)
      {
  ...
  {
    GLfloat glmvm[16];
    glGetFloatv(GL_PROJECTION_MATRIX, &glmvm[0]);
    std::stringstream ss;
    ss << "GL_PROJECTION_MATRIX" << std::endl;
    for (int r = 0; r < 4; ++r)
    {
      for (int c = 0; c < 4; ++c)
      {
  ...
  {
    GLfloat glmvm[16];
    glGetFloatv(GL_TEXTURE_MATRIX, &glmvm[0]);
    std::stringstream ss;
    ss << "GL_TEXTURE_MATRIX" << std::endl;
    for (int r = 0; r < 4; ++r)
    {
      for (int c = 0; c < 4; ++c)
      {
  ...
  GLfloat crp[4];
  glGetFloatv(GL_CURRENT_RASTER_POSITION, &crp[0]);
  GLfloat crd;
  glGetFloatv(GL_CURRENT_RASTER_DISTANCE, &crd);
  ignerr << "Raster Position: x: " << crp[0] << " y: " << crp[1] << " z: " << crp[2] << " w: " << crp[2] << std::endl;
  ignerr << "Raster Distance: " << crd << std::endl;
  ...
  {
    GLfloat crp[4];
    glGetFloatv(GL_CURRENT_TEXTURE_COORDS, &crp[0]);
    ignerr << "Texture Coord: x: " << crp[0] << " y: " << crp[1] << " z: " << crp[2] << " w: " << crp[2] << std::endl;
  }
